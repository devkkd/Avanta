...
export default function BulkUploadPage() {
  ...
  // Only filenames uploaded in this session — no thumbnails
  const [sessionFilenames, setSessionFilenames] = useState([]);
  ...
  const handleImagesChange = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        const newNames = Object.keys(data.data);
        setSessionFilenames(prev => {
          const existing = new Set(prev);
          return [...prev, ...newNames.filter(n => !existing.has(n))];
        });
        toast.success(`${newNames.length} image${newNames.length > 1 ? 's' : ''} uploaded to cloud!`);
      } else {
  ...
  const downloadTemplate = () => {
    ...
    const row1 = '"Elegant Anarkali Suit","Beautiful embroidered anarkali suit","anarkali-dupatta-sets","a-line-kalidar-suits","anarkali-red.jpg","anarkali-red-2.jpg,anarkali-red-3.jpg","M:10,L:15,XL:20,2XL:10,3XL:5","Georgette","Dry clean only","Red","#FF0000","AVT001-RED",TRUE,TRUE,TRUE,"wedding,festive",1';
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              {/* Filenames only — no thumbnails */}
              {sessionFilenames.length > 0 && (
                  ...
                  {sessionFilenames.map((filename) => (
                    <div key={filename} className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                      ...
                      <button
                        ...
                        className={`shrink-0 flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg transition font-medium ${
                          copiedName === filename
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <button onClick={() => setShowGuide(!showGuide)} className="w-full flex items-center justify-between px-5 py-4 text-left">
            <span className="font-semibold text-gray-700 flex items-center gap-2">
              <AlertCircle size={16} className="text-blue-500" /> How to fill the spreadsheet
            </span>
          ...
          {showGuide && (
            <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-3">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
                <strong>Tip:</strong> Upload images once in Step 2. Copy the filename. Next time just write the filename in CSV — system finds the image automatically from cloud. No re-upload ever needed.
              </div>
            </div>
          )}
        </div>
